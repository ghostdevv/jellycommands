...
export const registerCommands = async (
	...
	const registeredGlobalCommands = await client.$fetch<
		APIApplicationCommand[]
	>(Routes.applicationCommands(clientId), {
		method: 'PUT',
		body: globalCommandsArray.map((c) => c.applicationCommandData),
	});
	...
	for (const [guildId, commands] of guildCommands) {
		...
		const res = await client.$fetch<APIApplicationCommand[]>(
			...
			{
				method: 'PUT',
				body: commandsArray.map((c) => c.applicationCommandData),
			},
		);
