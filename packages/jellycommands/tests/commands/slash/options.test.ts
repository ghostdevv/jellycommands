...
describe('command options schema', () => {
	...
	it('rejects invalid names', () => {
		...
		expect(() =>
			commandSchema.parse({ name: 'NAME', description }),
		).toThrowError();
		expect(() =>
			commandSchema.parse({ name: 'spaces are wrong', description }),
		).toThrowError();
		expect(() =>
			commandSchema.parse({ name: 'no'.repeat(200), description }),
		).toThrowError();
		expect(() =>
			commandSchema.parse({ name: 'a'.repeat(33), description }),
		).toThrowError();
	});
	...
	it('rejects invalid descriptions', () => {
		...
		expect(() =>
			commandSchema.parse({ name, description: 'no'.repeat(200) }),
		).toThrowError();
		expect(() =>
			commandSchema.parse({ name, description: 'a'.repeat(101) }),
		).toThrowError();
	});
