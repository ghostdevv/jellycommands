...
:::note
If you select TypeScript in the template your code will be run with [`tsx`](https://www.npmjs.com/package/tsx) to avoid the compilation step. If you're on Node 22 you should consider using [--experimental-strip-types](https://nodejs.org/en/learn/typescript/run-natively), or Node 24 which works with [TypeScript erasable syntax](https://nodejs.org/api/typescript.html#type-stripping) out of the box.
:::
